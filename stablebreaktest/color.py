...
def cprint(*args, color="default", style="normal", bg_color=None, reset=True, timestamp=False, line=None, tqdm_desc=False, timestamp_format='%Y-%m-%d %H:%M:%S', prefix=None, suffix=None, timezone=None):
    ...
    Args:
        *args (Any): Text to be printed.
        color (str): Text color. Default is "default". Available options are: "default", "black", "red", "green", "yellow",
                     "blue", "purple", "cyan", "white", "flat_red", "flat_yellow", "flat_blue", "flat_purple", "flat_orange",
                     "flat_green", "flat_gray", "flat_cyan", "flat_pink".
        style (str): Text style. Default is "normal". Available options are: "normal", "bold", "italic", "underline",
                     "blink", "inverse", "strikethrough".
        bg_color (str): Background color. Default is None. Available options are the same as the color options.
        reset (bool): Whether to reset color after printing. Default is True.
        timestamp (bool): If set to True, it will add a timestamp in the beginning of the print. Default is False.
        line (int): If provided, will print equal number of lines before the text. Default is None.
        tqdm_desc (bool): If used as a description in tqdm. Default is False.
        timestamp_format (str): The format of the timestamp if timestamp is True. Default is '%Y-%m-%d %H:%M:%S'.
        prefix (Any): Optional prefix for the text. Default is None.
        suffix (Any): Optional suffix for the text. Default is None.
        timezone (str): The timezone to use for the timestamp. If None, the local timezone will be used.
...
def print_line(length, color="default", style="normal", bg_color=None, reset=True):
    ...
    Args:
        ...
        color (str): Text color. Default is "default". Available options are the same as in cprint().
        style (str): Text style. Default is "normal". Available options are the same as in cprint().
        bg_color (str): Background color. Default is None. Available options are the same as in cprint().
        reset (bool): Whether to reset color after printing. Default is True.
